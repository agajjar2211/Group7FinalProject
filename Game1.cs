...
namespace FinalProjectGame
{
	public class Game1 : Game
	{
        ...
        private void DrawGameOverMenu()
        {
            ...
            _spriteBatch.DrawString(_font, "Game Over", new Vector2(350, 150), Color.Red);

            // Draw the score
            _spriteBatch.DrawString(_font, $"Score: {_score} coins collected", new Vector2(320, 250), Color.White);

            // Draw the elapsed time
            _spriteBatch.DrawString(_font, $"Time Elapsed: {_elapsedTime:F1} seconds", new Vector2(310, 300), Color.White);

            // Draw Restart button (as a texture with text)
            _spriteBatch.Draw(_platformTexture, new Vector2(300, 400), Color.DarkGray);
            _spriteBatch.DrawString(_font, "Restart", new Vector2(350, 415), Color.White);

            // Draw Exit button (as a texture with text)
            _spriteBatch.Draw(_platformTexture, new Vector2(300, 500), Color.DarkGray);
